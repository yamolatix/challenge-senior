    ...
    describe("Modelo Message", () => {
      ...
      describe("funcionalidad", () => {
        ...
        beforeEach("Seed users", () => {
          ...
          return User.bulkCreate(users, { returning: true }).then(
            (createdUsers) => {
        ...
        beforeEach("Seed messages", () => {
          ...
          return Message.bulkCreate(messages, { returning: true }).then(
            (createdMessages) => {
          ...
          describe("truncateSubject", () => {
            ...
            it("retorna el objeto mensaje completo pero con el texto del subject limitado basado en el número pasado para determinar su length", () => {
              ...
              const messageWithTruncatedSubject =
                testMessage.truncateSubject(12);
              expect(messageWithTruncatedSubject).to.be.an("object");
              expect(messageWithTruncatedSubject.body).to.be.equal(
                `But I'm somewhere in that zone`
              );
              expect(messageWithTruncatedSubject.subject).to.be.equal(
                `I don't know`
              );
            });
      ...
      describe("messages", () => {
        ...
        it("sirve todos los mensajes de un remitente especifico en GET /from/{{senderId}}", () => {
          return agent
            ...
            .then((res) => {
              expect(res.body).to.be.an("array");
              expect(res.body.length).to.be.equal(2);
              expect(res.body).to.contain.a.thing.with.property(
                "body",
                "HEYOOOOOOO"
              );
        ...
        it(`sirve todos los mensajes de un remitente especifico en GET /from/{{senderId}}
          ...
          return agent
            ...
            .then((res) => {
              ...
              expect(
                getAllWhereSenderSpy.calledWith(obama.toString())
              ).to.be.equal(true);
        ...
        it("agrega un nuevo mensaje en adds POST /, respondiendo con un 201 y creando el mensaje", () => {
          return agent
            ...
            .then((foundMessage) => {
              expect(foundMessage.body).to.be.equal(
                "You are my best friend. I hope you know that."
              );
            });
        });
      });
